...
- `Detailed` display mode uses the structured per-frame item renderer. Do not flatten it into a text editor unless a later scope explicitly targets detailed-mode text selection/highlighting.
- `Plain` and `Slim` display modes use AvaloniaEdit-backed aggregate text surfaces so users can select text across messages. `Plain` renders a continuous plain text stream without preserving one-line-per-frame boundaries. `Slim` renders the same plain text payloads but keeps frame starts visible with `TX` / `RX` in a separate gutter column instead of embedding direction tags in the selectable payload text.
- STR aggregate rendering treats CR/LF as line-break semantics, not visible tokens: normalize `CRLF`, lone `CR`, and lone `LF` to real line breaks and do not show end-of-line markers by default. In `Slim`, the gutter marks the first visual line of each frame; payload-internal continuation lines keep the gutter blank. Hex rendering is already byte-visible and should not apply STR line-break normalization.
- Slim gutter rendering must use the same AvaloniaEdit `TextView` as the payload through a custom left margin. Do not use a second synchronized text editor for gutter text; separate editors drift when word wrap, font fallback, or viewport calculations change.
- Message viewer runtime state must be isolated by session and display mode. Data loading and search remain canonical at the frame/window level; per-mode render data such as aggregate documents, gutter text, frame-to-offset mappings, scroll anchors, and selection anchors must not leak between sessions or change spool/archive consumption semantics.
- Aggregate editor internals such as nested `ScrollViewer` may not exist before the control template is applied. Do not create dispatcher retry loops while waiting for template parts; attach synchronization only from explicit lifecycle events such as visual-tree attachment, display-mode changes, or later user/scroll activity.
